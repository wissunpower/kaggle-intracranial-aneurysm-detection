...
class BaseGateway:
    ...
    def _standardize_and_validate_paths(self, input_paths: List[Union[str, pathlib.Path]]) -> Tuple[List[str], List[str]]:
        ...
        output_paths = [os.path.normpath(output_dir + path[len(os.path.commonprefix([output_dir, path])):]) for path in input_paths]
    ...
    def share_files(
        ...
        for in_path, out_path in zip(input_paths, output_paths):
            ...
            if IS_RERUN:
                ...
                subprocess.run(f'mklink /d {out_path} {in_path}', shell=True, check=True)
        return output_paths
